...
// js/core, js/services, js/ui in small safe phases.
    ...
    const _canonicalBranch = (value, fallback = 'CS1') => {
        if (window.BranchIdentity && typeof window.BranchIdentity.normalize === 'function') {
            return window.BranchIdentity.normalize(value, { fallback });
        }
    ...
    const _branchAliases = (value) => {
        if (window.BranchIdentity && typeof window.BranchIdentity.aliases === 'function') {
            return window.BranchIdentity.aliases(value);
        }
...
>>>>>>> parent of 4757e42 (upload)
    ...
    window.getBranchNameDisplay = (code) => {
        const canonicalCode = _canonicalBranch(code, String(code || '').trim());
        for(let _bi = 1; _bi <= 10; _bi++) {
            if(canonicalCode === 'CS' + _bi) return clubConfig['branchName' + _bi] || ('Cơ sở ' + _bi);
        }
    ...
    window.applyClubConfigUI = () => {
        ...
        ['att_branch', 'att_month_branch'].forEach(function(selId) {
            ...
            if (window.userRole === 'coach' && window.coachBranch) {
                ...
                // Chỉ Admin được xem và lọc toàn bộ cơ sở
        ...
        // Phase 4K-6V4B: mỗi HLV phải thuộc đúng một cơ sở.
        // Không còn option "Tất cả cơ sở" vì đó không thể là security boundary an toàn.
        ...
        if (coachBranchEl) {
            ...
            if (!coachBranchEl.value) coachBranchEl.value = 'CS1';
        }
    };
  ...
  match /databases/{database}/documents {
    ...
    async function initSaaSDatabase(clubId) {
        ...
        if (window.userRole === 'coach') {
            ...
            // Phase 4K-6V4B: Coach runtime chỉ dùng Điểm danh và một cơ sở canonical.
    ...
    const _normalizeAuthContext = (input = {}) => {
        const role = _normalizeAuthRole(input.role || '');
        return {
            uid: String(input.uid || '').trim(),
            role,
            clubId: String(input.clubId || '').trim(),
            coachBranch: role === 'coach' ? _canonicalBranch(input.coachBranch || input.branch || '', '') : '',
        };
    };
    const _sameAuthContext = (a, b) => {
        const left = _normalizeAuthContext(a);
        const right = _normalizeAuthContext(b);
        return left.uid === right.uid && left.role === right.role && left.clubId === right.clubId && left.coachBranch === right.coachBranch;
    };
    ...
    const _getAuthCache = (uid) => {
        try {
            ...
            if (!d || d.uid !== uid || (Date.now() - Number(d.ts || 0)) >= _AUTH_CACHE_TTL_MS) return null;
            const ctx = _normalizeAuthContext(d);
            if (!ctx.role || (ctx.role !== 'super_admin' && !ctx.clubId)) return null;
            return { ...ctx, ts: d.ts };
        } catch(e) {}
        return null;
    };
    const _clearAuthCache = () => {
        try {
            localStorage.removeItem(_AUTH_CACHE_KEY);
            localStorage.removeItem('_qlclb_auth_v2');
        } catch(e) {}
    };
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        setTimeout(() => { if(typeof window._checkMonthlyReminder === 'function') window._checkMonthlyReminder(); }, 300);
                    ...
                    getDoc(doc(db, "users", user.uid)).then(async userDoc => {
                        ...
                        const _freshRole = (user.email && user.email.toLowerCase() === "admin@tstquynhon.com")
                            ? 'super_admin'
                            : (_ud.role || '');
...
Các giao dịch đã nhập với danh mục này vẫn giữ nguyên, chỉ xóa khỏi danh sách lựa chọn.`)) return;
    ...
    window.addNewStudent = async () => {
        ...
        const isSingleBranch = (clubConfig.branchCount === 1); const branch = isSingleBranch ? 'CS1' : _canonicalBranch(document.getElementById('add_branch').value, 'CS1');
...
(function setupMiCurrencyAll() {
    ...
    window.createCoachAccount = async () => {
        ...
        const branch = _canonicalBranch(branchEl ? branchEl.value : '', '');

        if (!name)  return alert('Vui lòng nhập tên HLV!');
        if (!email) return alert('Vui lòng nhập email!');
        if (!branch) return alert('Vui lòng chọn một cơ sở phụ trách cho HLV!');
        ...
        try {
            if (!note) {
                ...
                const payload = {
                    ...
                    coachId:     uid,
                    coachName:   '',
                    branch:      _canonicalBranch(window.coachBranch || '', ''),
                ...
                try {
                    ...
                    if (coachDoc.exists()) {
                        ...
                        payload.branch    = payload.branch || _canonicalBranch(coachDoc.data().branch || '', '');
                    }
                } catch(_) {}
    ...
    window.migrateCoachAccounts = async () => {
        if (!['admin', 'owner', 'super_admin'].includes(window.userRole)) {
            return alert('Chỉ Admin/Owner mới có quyền thực hiện chức năng này!');
        }
        ...
        try {
            ...
            for (const coachDoc of coachesSnap.docs) {
                ...
                const canonicalBranch = _canonicalBranch(data.branch || data.coachBranch || '', 'CS1');
                const needsFix = !data.uid || !data.email || !data.clubId || !data.role || data.branch !== canonicalBranch;
                if (needsFix) {
                    try {
                        await setDoc(doc(db, 'clubs', currentClubId, 'coaches', uid), {
                            uid,
                ...
                // Đồng bộ cả user thiếu và user có role/club/branch cũ sai.
                try {
                    const userRef = doc(db, 'users', uid), uSnap = await getDoc(userRef);
            ...
            if (btn) { btn.disabled = false; btn.textContent = '🔄 Đồng bộ tài khoản HLV cũ'; }
            const syncTitle = failed ? '⚠️ Đồng bộ chưa hoàn tất!' : '✅ Đồng bộ hoàn tất!';
            alert(syncTitle + `\n\n• Chuẩn hóa hồ sơ HLV: ${fixed}\n• Đồng bộ users/{uid}: ${userSynced}\n• Đã đúng (bỏ qua): ${skipped}\n• Không đồng bộ được: ${failed}\n\n` + (failed ? 'Chưa cho HLV đăng nhập; kiểm tra Rules và chạy lại đến khi lỗi bằng 0.' : 'Các HLV đã có hồ sơ phân quyền và có thể đăng nhập.'));
        } catch(e) {
            if (btn) { btn.disabled = false; btn.textContent = '🔄 Đồng bộ tài khoản HLV cũ'; }
...
<<<<<<< HEAD
        if (_isCoachScopedRuntimeSession()) {
            const result = {
                clubId: _clubId,
                source: 'coach-scoped',
                primary: { profilesHasDocs: 'coach-branch-scoped', transactionsHasDocs: 'not-probed', inventoryHasDocs: 'not-probed' },
                legacy:  { profilesHasDocs: 'not-probed', transactionsHasDocs: 'not-probed', inventoryHasDocs: 'not-probed' },
                reason:  'Coach session uses branch-scoped attendance roster; skip full-club data-source probes',
                safeToRender: true
            };
            window.__store && (window.__store.activeDataSource = 'coach-scoped');
            window.__firestoreDataSourceMetrics.source = 'coach-scoped';
            window.__firestoreDataSourceMetrics.reason = result.reason;
            window.__firestoreDataSourceMetrics.checkedAt = Date.now();
            console.info('[resolveActiveDataSource] Coach scoped session — skip full-club probes');
            return result;
        }
