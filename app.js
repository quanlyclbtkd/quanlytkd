...
window.invCustomCategories = [];
    ...
    function dispatchAppContextReady(reason) {
        // Phase 4J-8A: mark context-ready
        if (typeof markLoginPerf === 'function') markLoginPerf('context-ready');
...
// ── Phase 4J-8A: Login Performance Metrics ───────────────────────────────────
...
/**
 * Ghi mark thời điểm trong login flow.
 * @param {string} name — tên mark (ví dụ: 'login-submit', 'context-ready')
 */
function markLoginPerf(name) {
    if (!window.__loginPerfMetrics) return;
    window.__loginPerfMetrics.marks[name] = performance.now();
}

/**
 * Tính duration giữa hai marks và lưu.
 * @param {string} name  — tên duration
 * @param {string} start — tên mark bắt đầu
 * @param {string} end   — tên mark kết thúc
 */
function measureLoginPerf(name, start, end) {
    const marks = window.__loginPerfMetrics && window.__loginPerfMetrics.marks;
    if (marks && marks[start] !== undefined && marks[end] !== undefined) {
        window.__loginPerfMetrics.durations[name] = Math.round(marks[end] - marks[start]);
    }
}

/**
 * In toàn bộ login performance metrics ra console.
 * Gọi sau đăng nhập: window.printLoginPerformance()
 */
window.printLoginPerformance = function printLoginPerformance() {
    const m = window.__loginPerfMetrics || {};
    console.group('[LoginPerf] Phase 4J-8A — Login Performance');
    if (m.durations && Object.keys(m.durations).length > 0) {
        console.table(m.durations);
    } else {
        console.log('  (Chưa có duration — gọi sau khi đăng nhập xong)');
    }
    if (m.marks && Object.keys(m.marks).length > 0) {
        const t0 = m.marks['login-submit'] || m.marks['auth-state-received'] || 0;
        const formatted = {};
        Object.keys(m.marks).forEach(k => {
            formatted[k] = { 'ms since login-submit': Math.round(m.marks[k] - t0) + 'ms' };
        });
        console.group('Marks (relative to login-submit):');
        console.table(formatted);
        console.groupEnd();
    }
    if (m.warnings && m.warnings.length > 0) {
        console.group('Warnings:');
        m.warnings.forEach(w => console.warn(' ·', w));
        console.groupEnd();
    }
    console.groupEnd();
    return m;
    ...
    window.scheduleRender = () => {
        ...
        renderTimeout = setTimeout(function() {
            renderApp();
            // Phase 4J-8A: mark first tab render
            if (typeof markLoginPerf === 'function') {
                markLoginPerf('first-current-tab-rendered');
                measureLoginPerf('login-to-first-tab-render', 'login-submit', 'first-current-tab-rendered');
                measureLoginPerf('auth-to-ui-shell', 'auth-state-received', 'first-ui-shell-visible');
                measureLoginPerf('ui-shell-to-tab-render', 'first-ui-shell-visible', 'first-current-tab-rendered');
            }
        }, 250);
    };
    ...
    window.applyClubConfigUI = () => {
        ...
        // [Phase 4.0C-1A] Backup guard: clear "Đang tải dữ liệu CLB…" khi settings đã load
        // Phòng trường hợp _clubCb chạy sau applyClubConfigUI (race condition)
        const _subElCfg = document.getElementById('displaySubtitle');
        if (_subElCfg && _subElCfg.innerText.indexOf('Đang tải') !== -1) {
            _subElCfg.innerText = 'Hệ thống quản lý thu chi CLB và Báo cáo thuế HKD - by Tình Trương';
        }
        // [Phase 4.0C-1A] App đã load thành công → tự động ẩn banner lỗi js/main.js
        // Nếu app.js hoạt động tốt (settings snapshot nhận được), main.js không critical
        try {
            const _errBanner = document.getElementById('__mainjs_load_error');
            if (_errBanner) {
                _errBanner.style.transition = 'opacity 0.5s';
                _errBanner.style.opacity = '0';
                setTimeout(function() { _errBanner.remove(); }, 600);
            }
        } catch(_e) {}
    };
    ...
    window.handleLogin = async () => {
        ...
        try {
            // Phase 4J-8A: mark login submit time
            if (typeof markLoginPerf === 'function') markLoginPerf('login-submit');
            ...
            // Phase 4J-8A: mark firebase auth success
            if (typeof markLoginPerf === 'function') markLoginPerf('firebase-auth-success');
    ...
    window.loadSuperAdminData = async function() {
        ...
        if (window.ensureSuperAdminModule) {
            const loaded = await window.ensureSuperAdminModule('loadSuperAdminData');
        ...
        console.warn('[SuperAdminFallback] loadSuperAdminData: module not loaded');
        // [Phase 4.0C-1A] Hiển thị lỗi rõ ràng trong UI — không để kẹt "Đang tải dữ liệu..." vô hạn
        const _saListEl = document.getElementById('sysClubListMain');
        if (_saListEl) {
            const _isFileProt = window.location.protocol === 'file:' || window.__APP_STANDALONE_FILE_MODE;
            if (_isFileProt) {
                _saListEl.innerHTML =
                    '<div style="text-align:center;padding:36px 24px;">' +
                    '<div style="font-size:2.2rem;margin-bottom:10px;">🚫</div>' +
                    '<div style="font-size:0.92rem;font-weight:900;color:#dc2626;margin-bottom:12px;">Đang mở bằng file:// — ES modules bị chặn bởi trình duyệt</div>' +
                    '<div style="font-size:0.82rem;color:#475569;line-height:2;text-align:left;display:inline-block;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:14px 18px;">' +
                    '<b>Cách sửa:</b><br>' +
                    '1. Mở Terminal (CMD / PowerShell) trong thư mục project<br>' +
                    '2. Chạy lệnh: <code style="background:#1e293b;color:#86efac;padding:2px 10px;border-radius:6px;font-weight:700;">npm run local</code><br>' +
                    '3. Mở trình duyệt tại: <code style="background:#1e293b;color:#93c5fd;padding:2px 10px;border-radius:6px;font-weight:700;">http://localhost:8000</code><br>' +
                    '4. <b>Không</b> mở file index.html trực tiếp từ File Explorer' +
                    '</div></div>';
            } else {
                _saListEl.innerHTML =
                    '<div style="text-align:center;padding:36px 24px;">' +
                    '<div style="font-size:2rem;margin-bottom:10px;">⚠️</div>' +
                    '<div style="font-size:0.9rem;font-weight:900;color:#b45309;margin-bottom:10px;">Module SuperAdmin chưa load được</div>' +
                    '<div style="font-size:0.8rem;color:#475569;margin-bottom:16px;">Kiểm tra Console (F12) để biết chi tiết lỗi.</div>' +
                    '<button onclick="window.loadSuperAdminData()" style="padding:10px 24px;background:#4338ca;color:#fff;border:none;border-radius:10px;font-weight:800;cursor:pointer;font-size:0.85rem;">🔄 Thử lại</button>' +
                    '</div>';
            }
        }
        if (window.showToast) window.showToast('Module SuperAdmin chưa sẵn sàng. Vui lòng chạy bằng http://localhost:8000', 5000);
    };
  ...
  match /databases/{database}/documents {
    ...
    window.saDeleteTransactions = async () => {
        ...
        try {
            ...
            // Phase 4J-8A: Cursor pagination — xóa tất cả tx trước ngày, không bị cap 500
            // [FIXED] UNSAFE_LIMIT_FOR_CALCULATION → cursor pagination
            const PAGE_DEL = 400;
            const MAX_PAGES_DEL = 100; // safety: tối đa 40.000 tx
            let _deleteCursor = null;
            let _deletePages  = 0;
            let deleted = 0;
            let total   = 0;
            resultEl.innerHTML = '<div style="color:#0033A0;font-weight:700;font-size:0.82rem;margin-top:10px;">⏳ Đang đếm giao dịch...</div>';

            while (_deletePages < MAX_PAGES_DEL) {
                const _constraints = [
                    where('date', '<', beforeDate),
                    orderBy('date'),
                    limit(PAGE_DEL),
                ];
                if (_deleteCursor) _constraints.push(startAfter(_deleteCursor));
                const snap = await getDocs(query(txRef, ..._constraints));
                if (snap.empty) break;
                total += snap.size;
                const batch = writeBatch(db);
                ...
                await batch.commit();
                deleted += snap.size;
                resultEl.innerHTML = `<div style="color:#0033A0;font-weight:700;font-size:0.82rem;margin-top:10px;">⏳ Đã xóa ${deleted} giao dịch...</div>`;
                if (snap.size < PAGE_DEL) break;
                _deleteCursor = snap.docs[snap.docs.length - 1];
                _deletePages++;
            }
            if (deleted === 0) {
    ...
    async function initSaaSDatabase(clubId) {
        // Phase 4J-8A: mark initSaaSDatabase start
        if (typeof markLoginPerf === 'function') markLoginPerf('initSaaSDatabase-start');
        ...
        // Phase 4J-8A: UI shell is now visible (login overlay hidden, main app shown)
        if (typeof markLoginPerf === 'function') markLoginPerf('first-ui-shell-visible');
        // Phase 4J-8A: Hiện thị loading message sớm — tránh cảm giác app đơ trên mobile
        const _loadingSubtitleEl = document.getElementById('displaySubtitle');
        if (_loadingSubtitleEl && window.userRole !== 'super_admin') {
            _loadingSubtitleEl.innerText = 'Đang tải dữ liệu CLB…';
        }
        ...
        const _clubCb = (snap) => {
            ...
            if(snap.exists()) {
                ...
                // [Phase 4.0C-1A] Clear "Đang tải dữ liệu CLB…" khi club data đã load
                const _subEl = document.getElementById('displaySubtitle');
                if (_subEl && _subEl.innerText.indexOf('Đang tải') !== -1) {
                    _subEl.innerText = 'Hệ thống quản lý thu chi CLB và Báo cáo thuế HKD - by Tình Trương';
                }
        ...
        // UNSAFE_LIMIT_FOR_CALCULATION — inventory listener cắt tại 500 bản ghi gần nhất.
        // allInventory vẫn dùng cho tab Kho/lịch sử gần đây (display) — OK với limit 500.
        // Công nợ kho đồ dùng _loadAllUnpaidInvDebts() riêng — lấy TẤT CẢ unpaid, không bị limit.
    ...
    window.loadSARevenue = async () => {
        ...
        try {
            ...
            const revenueList = await Promise.all(clubDocs.map(async (docSnap) => {
                ...
                try {
                    // [3.8D-Phase6] UNSAFE_LIMIT_FOR_CALCULATION: SA revenue limit(1000) per CLB.
                    // CLB lớn > 1000 tx/kỳ sẽ thiếu doanh thu SuperAdmin. Chưa sửa vì query nhiều CLB song song.
                    if (typeof window.warnUnsafeLimit === 'function') window.warnUnsafeLimit('superAdmin:revenue:limit1000:' + cid, 'sa-revenue-per-club');
                    const txSnap = await getDocs(query(
                        collection(db, "clubs", cid, "transactions"),
                        where("date", ">=", startDate),
                        where("date", "<=", endDate),
                        limit(1000) // [3.3E] SA revenue: cap per club per period — [3.8D-Phase6] chờ paginate từng CLB ở phase tiếp theo
                    ));
                    let total = 0;
                    txSnap.forEach(tx => {
                        const t = tx.data();
    ...
    window.ppLookupLogin = async () => {
        ...
        try {
            ...
            if (!_profSnap.exists()) {
                // Phase 4J-8A: Thử query server-side theo searchName trước (tránh full scan)
                // [FIXED] UNSAFE_LIMIT_FOR_CALCULATION → server-side searchName query
                let _foundViaSearch = false;
                try {
                    const _searchSnap = await getDocs(query(
                        collection(_dbForParent, 'clubs', _clubId, 'profiles'),
                        orderBy('searchName'),
                        startAt(_normInput),
                        endAt(_normInput + ''),
                        limit(5)
                    ...
                    let _foundDoc = null;
                    _searchSnap.forEach(d => {
                        if (!_foundDoc && _normalizeVN(d.id) === _normInput) _foundDoc = d;
                        else if (!_foundDoc && _normalizeVN((d.data().searchName || d.id)) === _normInput) _foundDoc = d;
                    });
                    if (_foundDoc) {
                        _profSnap   = _foundDoc;
                        _actualName = _foundDoc.id;
                        _foundViaSearch = true;
                    }
                } catch (_sErr) {
                    // searchName index không tồn tại — fallback sang paginated scan
                }

                // Fallback: paginated scan nếu searchName index chưa có
                if (!_foundViaSearch) {
                    if (typeof window.warnUnsafeLimit === 'function') window.warnUnsafeLimit('parentClub:profileScan:noIndex', 'parent-club-profile-scan-fallback');
                    const _scanPage = 300;
                    let _scanCursor = null;
                    let _pagesDone  = 0;
                    const _maxScanPages = 10; // tối đa 3000 profiles
                    let _scanFoundDoc = null;
                    while (_pagesDone < _maxScanPages) {
                        const _scanConstraints = [orderBy('__name__'), limit(_scanPage)];
                        if (_scanCursor) _scanConstraints.push(startAfter(_scanCursor));
                        const _scanSnap = await getDocs(query(collection(_dbForParent, 'clubs', _clubId, 'profiles'), ..._scanConstraints));
                        _scanSnap.forEach(d => {
                            if (!_scanFoundDoc && _normalizeVN(d.id) === _normInput) _scanFoundDoc = d;
                        });
                        if (_scanFoundDoc || _scanSnap.size < _scanPage) break;
                        _scanCursor = _scanSnap.docs[_scanSnap.docs.length - 1];
                        _pagesDone++;
                    }
                    if (!_foundViaSearch && _scanFoundDoc) {
                        _profSnap   = _scanFoundDoc;
                        _actualName = _scanFoundDoc.id;
                    }
                    if (_pagesDone >= _maxScanPages) {
                        console.warn('[Scale] Parent-club profile scan đạt max pages (3000 profiles scanned). Cần backfill searchName index.');
                    }
                }
            }
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            try {
                // Phase 4J-8A: mark when auth state fires
                if (typeof markLoginPerf === 'function') markLoginPerf('auth-state-received');
    ...
    window.listenToData = (monthStr) => {
        ...
        // UNSAFE_LIMIT_FOR_CALCULATION — CLB 1.000+ võ sinh có thể > 500 giao dịch/tháng.
        // → Dashboard/báo cáo tháng có thể thiếu dữ liệu nếu tháng > 500 giao dịch.
        // → Dùng window.loadTransactionsForPeriod({ colRef, month, forExport: true }) cho export/báo cáo.
...
Các giao dịch đã nhập với danh mục này vẫn giữ nguyên, chỉ xóa khỏi danh sách lựa chọn.`)) return;
    ...
    window.updateProfile = async () => {
        ...
        try {
            if (oldName !== newName) {
                ...
                // Phase 4J-8A: Paginated tx scan cho rename — không bị cap 500
                // [FIXED] UNSAFE_LIMIT_FOR_CALCULATION → cursor pagination
                const _renamePage = 400;
                let _renameCursor = null;
                let _renamePagesDone = 0;
                const _MAX_RENAME_PAGES = 50;
                const _txDocsToUpdate = [];

                while (_renamePagesDone < _MAX_RENAME_PAGES) {
                    const _renameConstraints = [
                        where("description", ">=", oldName),
                        where("description", "<=", oldName + '\uf8ff'),
                        orderBy("description"),
                        limit(_renamePage),
                    ];
                    if (_renameCursor) _renameConstraints.push(startAfter(_renameCursor));
                    const _renameSnap = await getDocs(query(colRef, ..._renameConstraints));
                    _renameSnap.forEach(tDoc => {
                        ...
                        if (needsUpdate) _txDocsToUpdate.push({ id: tDoc.id, desc: updatedDesc });
                    });
                    if (_renameSnap.size < _renamePage) break;
                    _renameCursor = _renameSnap.docs[_renameSnap.docs.length - 1];
                    _renamePagesDone++;
                }
                // Commit tất cả tx updates cùng batch set/delete profile
                _txDocsToUpdate.forEach(({ id, desc }) => {
                    batch.update(doc(db, "clubs", currentClubId, "transactions", id), { description: desc });
                });
    ...
    window.deleteTx = async (id, relatedInvId) => { 
        if(window.userRole !== 'viewer' && confirm("⚠️ Bạn có chắc muốn xóa giao dịch này? (Nếu là giao dịch kho sẽ không tự hoàn trả số dư, hãy chủ động cập nhật lại kho sau khi xóa)")) { 
            ...
            if (txToDelete && (txToDelete.type === 'Học phí' || txToDelete.type === 'Học phí + Lệ phí thi')) {
                ...
                // Phase 4J-8A: Paginated query — không bị cap 500 cho võ sinh nhiều năm
                // [FIXED] UNSAFE_LIMIT_FOR_CALCULATION → cursor pagination
                const _stuPage    = 400;
                const _stuMaxPg   = 50;
                let _stuCursor    = null;
                ...
                while (_stuPgsDone < _stuMaxPg) {
                    const _stuConstraints = [
                        where("description", "==", studentName),
                        orderBy("timestamp", "desc"),
                        limit(_stuPage),
                    ];
                    if (_stuCursor) _stuConstraints.push(startAfter(_stuCursor));
                    const _stuTxSnap = await getDocs(query(colRef, ..._stuConstraints));
                    _stuTxSnap.forEach(tDoc => {
                    ...
                    if (_stuTxSnap.size < _stuPage) break;
                    _stuCursor = _stuTxSnap.docs[_stuTxSnap.docs.length - 1];
                    _stuPgsDone++;
                }
                    // (remainingMonths populated above by paginated loop)
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        document.getElementById('searchInput').oninput = () => { window._resetListPages && window._resetListPages(); if (window.invalidateCurrentTab) { window.invalidateCurrentTab('search-change'); } else { scheduleRender(); } }; 
...
(function setupMiCurrencyAll() {
    ...
    window.renderAttendanceList = async () => {
        ...
        try {
            const attRef = collection(db, 'clubs', currentClubId, 'attendance');
            // Phase 4J-8A: dùng attendanceDailyLimit từ scaleConfig
            const _attDailyLim = (window.__scaleConfig && window.__scaleConfig.attendanceDailyLimit) || 1200;
            const q = query(attRef, where('date', '==', _attCurrentDate), limit(_attDailyLim)); // [4J-8A] scale-safe daily limit
            const snap = await getDocs(q);
            _attendanceCache = {};
            snap.forEach(d => {
                const _sd = d.data();
