  ...
  match /databases/{database}/documents {
    ...
    window.saDeleteTransactions = async () => {
        ...
        try {
            ...
            // [3.8D-Phase6] UNSAFE_LIMIT_FOR_CALCULATION: batch delete dùng limit(500) — có thể bỏ sót tx cũ nếu CLB > 500 tx trước ngày đó.
            // Chưa sửa vì liên quan đến batch write logic, rủi ro cao khi refactor.
            if (typeof window.warnUnsafeLimit === 'function') window.warnUnsafeLimit('deleteTx:batchDelete:limit500', 'batch-delete-cap');
            const q = query(txRef, where('date', '<', beforeDate), limit(500)); // [3.3E] batch delete cap
            const snap = await getDocs(q);
            if (snap.empty) {
    ...
    window.ppLookupLogin = async () => {
        ...
        try {
            ...
            if (!_profSnap.exists()) {
                ...
                // [3.8D-Phase6] UNSAFE_LIMIT_FOR_CALCULATION: parent-club profile scan limit(500) — có thể bỏ sót nếu CLB > 500 hồ sơ.
                // Chưa sửa vì đây là fallback scan ít xảy ra, rủi ro thấp. Warning để theo dõi.
                if (typeof window.warnUnsafeLimit === 'function') window.warnUnsafeLimit('parentClub:profileScan:limit500', 'parent-club-profile-scan');
                const _allProfsSnap = await getDocs(query(collection(_dbForParent, 'clubs', _clubId, 'profiles'), limit(500))); // [3.3E] limit: parent-club profile scan
                let _foundDoc = null;
    ...
    window.listenToData = (monthStr) => {
        ...
        const qByDate    = query(colRef, where("date", ">=", start), where("date", "<=", end), orderBy("date", "desc"), limit(500));
        const qByTxMonth = query(colRef, where("txMonth", "==", monthStr), limit(500));
    ...
    window.updateProfile = async () => {
        ...
        try {
            if (oldName !== newName) {
                ...
                // [3.8D-Phase6] UNSAFE_LIMIT_FOR_CALCULATION: rename tx scan limit(500) — bỏ sót nếu võ sinh có > 500 giao dịch.
                // Chưa sửa vì rename là thao tác hiếm và scan theo description range. Warning để theo dõi.
                if (typeof window.warnUnsafeLimit === 'function') window.warnUnsafeLimit('students:renameTxScan:limit500', 'student-rename-tx-description-scan');
                const allOldTxSnap = await getDocs(query(colRef, where("description", ">=", oldName), where("description", "<=", oldName + '\uf8ff'), limit(500))); // [3.3E] limit: student rename tx scan
    ...
    window.deleteTx = async (id, relatedInvId) => { 
        if(window.userRole !== 'viewer' && confirm("⚠️ Bạn có chắc muốn xóa giao dịch này? (Nếu là giao dịch kho sẽ không tự hoàn trả số dư, hãy chủ động cập nhật lại kho sau khi xóa)")) { 
            ...
            if (txToDelete && (txToDelete.type === 'Học phí' || txToDelete.type === 'Học phí + Lệ phí thi')) {
                ...
                // [3.8D-Phase6] UNSAFE_LIMIT_FOR_CALCULATION: paidUntil recalc sau deleteTx dùng limit(500).
                // Võ sinh > 500 tx lịch sử (CLB lớn, nhiều năm) có thể tính sai paidUntil sau khi xóa tx cũ.
                // Chưa sửa: liên quan đến delete flow phức tạp. Warning để theo dõi.
                if (typeof window.warnUnsafeLimit === 'function') window.warnUnsafeLimit('deleteTx:paidUntilRecalc:limit500', 'delete-tx-paiduntil-recalc');
                const _stuTxSnap = await getDocs(query(colRef, where("description", "==", studentName), limit(500))); // [3.3E] limit: delete-tx paidUntil recalc
...
(function setupMiCurrencyAll() {
    ...
    window.renderAttendanceList = async () => {
        ...
        try {
            const attRef = collection(db, 'clubs', currentClubId, 'attendance');
            const q = query(attRef, where('date', '==', _attCurrentDate), limit(500)); // [3.3E] limit: att by date
            const snap = await getDocs(q);
            _attendanceCache = {};
            snap.forEach(d => {
                const _sd = d.data();
