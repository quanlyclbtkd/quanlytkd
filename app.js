    ...
    const { getFirestore, collection, doc, getDoc, onSnapshot, addDoc, updateDoc, deleteDoc, query, orderBy, where, writeBatch, setDoc, arrayUnion, arrayRemove, getDocs, limit, increment, getCountFromServer } = window._fb_init;
  ...
  match /databases/{database}/documents {
    ...
    window.saDeleteTransactions = async () => {
        ...
        try {
            ...
            const q = query(txRef, where('date', '<', beforeDate));
            const snap = await getDocs(q);
            if (snap.empty) {
            ...
            for (let i = 0; i < docs.length; i += batchSize) {
                ...
                resultEl.innerHTML = `<div style="color:#0033A0;font-weight:700;font-size:0.82rem;margin-top:10px;">⏳ Đã xóa ${deleted}/${total} giao dịch...</div>`;
            }
            resultEl.innerHTML = `<div style="color:#16a34a;font-weight:800;font-size:0.85rem;margin-top:10px;padding:10px 14px;background:#f0fdf4;border:1px solid #86efac;border-radius:10px;">✅ Đã xóa thành công <strong>${total} giao dịch</strong> trước ngày ${beforeDate} của CLB "${clubName}".</div>`;
            window.showToast(`✅ Đã xóa ${total} giao dịch!`);
            window.loadSuperAdminData();
        } catch (e) {
            console.error(e);
            resultEl.innerHTML = `<div style="color:#dc2626;font-weight:700;font-size:0.82rem;margin-top:10px;">❌ Lỗi: ${e.message}</div>`;
        } finally {
            btn.disabled = false; btn.innerText = '🗑️ Xóa Giao Dịch';
        }
    };
    ...
    async function initSaaSDatabase(clubId) {
        ...
        if (window.userRole === 'coach') {
            ...
            if (_attHeader) {
                ...
                const _branchName = window.coachBranch
                    ...
                    : 'Tất cả';
                _attHeader.innerHTML = `<span style="font-size:0.78rem;background:#e0f2fe;color:#0369a1;border:1px solid #bae6fd;padding:6px 14px;border-radius:99px;font-weight:700;">👨‍🏫 HLV đang điểm danh — Cơ sở: ${_branchName}</span>`;
            }
        ...
        activeListeners.push(onSnapshot(query(invRef, orderBy("timestamp", "desc"), limit(1000)), (snap) => { allInventory = snap.docs.map(d => ({id: d.id, ...d.data()})); scheduleRender(); }));
                ...
                try {
                    ...
                    txSnap.forEach(tx => {
                        ...
                        if (!EXPENSE_TYPES.includes(t.type)) {
                            // [SỬA ĐỒNG BỘ] Phân tích số tiền linh hoạt: xử lý cả number lẫn string có dấu phẩy/chấm
    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        setTimeout(() => { if(typeof window._checkMonthlyReminder === 'function') window._checkMonthlyReminder(); }, 300);
                    ...
                    let _found = false;
                    try {
                        const _allClubs = await getDocs(collection(db, 'clubs'));
    ...
    window.listenToData = (monthStr) => {
        ...
        const qByDate   = query(colRef, where("date", ">=", start), where("date", "<=", end), orderBy("date", "desc"), limit(300));
        const qByTxMonth = query(colRef, where("txMonth", "==", monthStr), limit(200));
...
Các giao dịch đã nhập với danh mục này vẫn giữ nguyên, chỉ xóa khỏi danh sách lựa chọn.`)) return;
    ...
    document.getElementById('transactionForm').onsubmit = async (e) => {
        ...
        if (type === 'Học phí' || type === 'Học phí + Lệ phí thi') {
            ...
            newPaidUntil = _lastRecorded > (profile.paidUntil || '') ? _lastRecorded : (profile.paidUntil || _lastRecorded);
        }
            ...
            if (txToDelete && (txToDelete.type === 'Học phí' || txToDelete.type === 'Học phí + Lệ phí thi')) {
                ...
                if (studentName) {
                    ...
                    allTransactions.forEach(t => {
                        if (t.id !== id && (t.type === 'Học phí' || t.type === 'Học phí + Lệ phí thi') && (t.description || '').trim() === studentName) {
                            if (t.packageMonths) {
                                remainingMonths.push(...t.packageMonths);
                            } else if (t.txMonth) {
                                remainingMonths.push(t.txMonth);
                            }
                        }
                    });
    ...
    window.quickPay = async (name, monthsStr, branch, defaultFee, skipPrompt) => {
        ...
        try {
            ...
            const _safeQPaidUntil = actualLastMonth > (profile.paidUntil || '') ? actualLastMonth : (profile.paidUntil || actualLastMonth);
            await setDoc(doc(db, "clubs", currentClubId, "profiles", cleanName), { status: 'active', branch: branch || 'CS1', belt: profile.belt || 'Đai trắng - Cấp 10', createdAt: profile.createdAt || getLocalToday(), paidUntil: _safeQPaidUntil, paidMonths: arrayUnion(...paidMonthsList) }, { merge: true });
    ...
    window.openQuickPayModal = (name, owedMonthsStr, branch) => {
        ...
        if (!modal) { window.quickPay(name, owedMonthsStr, branch, (feePerMonth * totalMonths).toString()); return; }
        ...
        customBtn.onclick = () => {
            modal.style.display = 'none';
            window.quickPay(name, owedMonthsStr, branch, feePerMonth > 0 ? String(feePerMonth * totalMonths) : '0');
        };
        ...
        try {
            if(action === 'pay') {
                ...
                if(n1 && f1 > 0) {
                    ...
                    // [FIX BÁO NỢ] Không thụt lùi paidUntil
                    const _cu1 = (allProfiles[n1] && allProfiles[n1].paidUntil) || '';
                    await setDoc(doc(db, "clubs", currentClubId, "profiles", n1), { paidUntil: m1 > _cu1 ? m1 : _cu1 }, { merge: true });
                }
                if(n2 && f2 > 0) {
                    ...
                    // [FIX BÁO NỢ] Không thụt lùi paidUntil
                    const _cu2 = (allProfiles[n2] && allProfiles[n2].paidUntil) || '';
                    await setDoc(doc(db, "clubs", currentClubId, "profiles", n2), { paidUntil: m2 > _cu2 ? m2 : _cu2 }, { merge: true });
                }
    ...
    window.exportReceipt = async (name, amount, type, date, txMonth, branch, extraDesc = '', receiptTitle = 'BIÊN LAI THU TIỀN', breakdown = null) => {
        ...
        try {
            ...
            if (amount > 0 && receiptTitle !== 'BIÊN LAI THU TIỀN') {
                ...
                const _qrUrl = generateVietQR(amount, qrName, paymentContent);
    ...
    function renderApp() {
        ...
        Object.keys(allProfiles).forEach(name => {
            ...
            if(p.status === 'active') {
                ...
                if(_curTabId === 'active') activeHtml += `<tr><td class="name-link text-[0.95rem]" onclick="openProfile('${safeNameEscaped}')">${_displayName(name)}${_listYrBadge}${_newBadge}${p.notes ? ' <span title="'+p.notes+'">📝</span>' : ''}${_activeNickBadge}</td><td class="text-[0.7rem] font-bold text-slate-500">${p.memberId || '-'}</td><td>${beltHTML}</td>${branchTdHTML}<td>${formatDate(p.dob)}</td><td>${paidBadge}</td><td class="font-medium text-slate-600">${safePhone}</td><td class="text-slate-500">${formatDate(p.createdAt)}</td><td><button type="button" class="btn-sm bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200" onclick="openProfile('${safeNameEscaped}')">${window.userRole === 'admin' ? '✏️ Sửa' : '👁️ Xem'}</button></td></tr>`;
                ...
                if (!p.feeExempt && (!p.skippedMonths || !p.skippedMonths.includes(selMonth))) {
                    if (!p.paidUntil || p.paidUntil < selMonth) {
                        let firstUnpaid = p.paidUntil ? addMonthsToYYYYMM(p.paidUntil, 1) : (p.createdAt ? p.createdAt.substring(0, 7) : selMonth);
                        let cur = firstUnpaid;
...
(function setupMiCurrencyAll() {
    ...
    window.showAttMemberHistory = async (idxOrName, overrideMonth) => {
        ...
        try {
            ...
            const qQ = query(attRef, where('profileId', '==', name), where('month', 'in', quarterMonths));
    ...
    window.renderAttendanceList = async () => {
        ...
        try {
            const attRef = collection(db, 'clubs', currentClubId, 'attendance');
            const q = query(attRef, where('date', '==', _attCurrentDate));
            const snap = await getDocs(q);
            _attendanceCache = {};
            snap.forEach(d => {
                const _sd = d.data();
    ...
    window.renderAttMonthly = async () => {
        ...
        try {
            const attRef  = collection(db, 'clubs', currentClubId, 'attendance');
            const q       = query(attRef, where('month', '==', selMonth));
            const snap    = await getDocs(q);
    ...
    window.exportAttendanceExcel = async () => {
        ...
        try {
            ...
            const attRef = collection(db, 'clubs', currentClubId, 'attendance');
            const snap   = await getDocs(query(attRef, where('month', '==', selMonth)));
    ...
    window.loadCoachAccounts = async () => {
        ...
        try {
            ...
            const snap = await getDocs(coachesRef);
            if (snap.empty) {
    ...
    window.setupNotifListener = () => {
        if (!currentClubId || (window.userRole !== 'admin' && window.userRole !== 'super_admin')) return;
        // Hủy listener cũ nếu có (tránh duplicate khi re-init)
