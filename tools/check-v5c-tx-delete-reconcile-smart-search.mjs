...
const build = 'tx-delete-reconcile-smart-search-20260703-v5c';
let pass = 0, fail = 0;
function check(name, ok) { if (ok) { pass++; console.log('✅', name); } else { fail++; console.error('❌', name); } }

console.log('\n=== Phase 4K-6V5C — TX Delete Reconcile + Smart Search ===\n');
...
check('canonical reconcile reads remaining student transactions from Firestore after delete', files.main.includes('Phase 4K-6V5C: đọc authoritative từ Firestore sau khi xóa') && files.main.includes("sdk.where('description', '==', studentName)"));
...
check('StudentSearchIndex has professional name token scoring', files.searchIndex.includes('name-token-exact') && files.searchIndex.includes('name-token-prefix') && files.searchIndex.includes('name-token-contains'));
...
check('package exposes V5C check', files.pkg.includes('check:v5c-tx-delete-reconcile-smart-search'));
