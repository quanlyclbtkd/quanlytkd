#!/usr/bin/env node
/** Phase 4K-6V4D7 — Coach Login + Quit Full List Recovery gate. */
...
check('APP_PATCH_VERSION marks V4D5', (files.app.includes("APP_PATCH_VERSION = '4K-6V4D7-coach-quit-attendance-full-recovery-20260630'") || files.app.includes("APP_PATCH_VERSION = '4K-6V4D8-coach-attendance-auth-roster-final-recovery-20260630'")) && (files.main.includes("APP_PATCH_VERSION = '4K-6V4D7-coach-quit-attendance-full-recovery-20260630'") || files.main.includes("APP_PATCH_VERSION = '4K-6V4D8-coach-attendance-auth-roster-final-recovery-20260630'")));
...
check('coach alias listener is branch-field-only', files.profiles.includes("const aliasQuery = fbQuery(profRef, fbWhere(spec.field, '==', spec.value));") && !files.profiles.includes("const aliasQuery = fbQuery(profRef, statusConstraint, fbWhere('branch', '==', alias));"));
...
check('public mirror is synced', files.publicProfiles.includes('coach-branch-field-alias-compat') && files.publicRenderStudents.includes('both web and mobile must prefer') && files.publicFinance.includes(`finance.service.js?v=${BUILD}`));
