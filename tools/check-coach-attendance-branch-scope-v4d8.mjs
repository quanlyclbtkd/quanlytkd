#!/usr/bin/env node
/** Phase 4K-6V4D8 — Coach Attendance Branch Scope Completeness */
...
check('Runtime cache marker bumped to V4D7 across entrypoints',
  (index.match(/coach-attendance-branch-rule-tolerant-20260630-v4d8/g) || []).length >= 5 &&
  main.includes('profiles.listeners.js?v=coach-attendance-branch-rule-tolerant-20260630-v4d8'));
...
check('Attendance client filter compares dynamic aliases/display names',
  attendance.includes('Phase 4K-6V4D8') &&
...
check('Firestore Rules allow only assigned branch aliases and configured names',
  ...
  rules.includes('function branchEquivalentInClub') &&
  rules.includes('function branchValueIsKnownInClub') &&
...
check('Coach fallback is permission-denied tolerant per alias spec',
  profiles.includes('function _isPermissionDenied') &&
  profiles.includes("_warnCoachSpecDenied('[ProfilesFallback] Coach branch fallback'") &&
  profiles.includes("okSpecs === 0 && Object.keys(activeMap).length === 0"));
...
check('No full-club profiles read is introduced for Coach',
  ...
  coachFallbackBody.includes(".catch(error => ({ spec, error }))") && coachFallbackBody.includes("_warnCoachSpecDenied('[ProfilesFallback] Coach branch fallback'"));
...
console.log('Phase 4K-6V4D8 coach attendance branch scope checks passed.\n');
