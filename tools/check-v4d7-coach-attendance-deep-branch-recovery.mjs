...
check('Configured branch display names normalize to canonical codes', files.branchIdentity.includes('function _codeFromConfiguredName') && files.branchIdentity.includes('const configuredCode = _codeFromConfiguredName(raw)'));
...
check('Legacy app fallback also uses branch-field safe specs', files.app.includes("const _coachFields=['branch','branchCode','coachBranch','branchName','facility','base','coso','coSo','location']") && files.app.includes('coach-branch-field-safe-fallback'));
...
check('Public mirror files are synced', files.publicAttendance.includes('function _profileBranchValue') && files.publicProfiles.includes('COACH_PROFILE_BRANCH_FIELDS') && files.publicBranchIdentity.includes('function _codeFromConfiguredName'));
