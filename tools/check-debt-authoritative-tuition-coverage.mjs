...
function chargeable(p, selected){ const sel=normalizeYYYYMM(selected); if(!sel||p.feeExempt===true) return []; const skipped=Array.isArray(p.skippedMonths)?p.skippedMonths.map(normalizeYYYYMM).filter(Boolean):[]; const paidMonths=Array.isArray(p.paidMonths)?p.paidMonths.map(normalizeYYYYMM).filter(Boolean):[]; const pu=normalizeYYYYMM(p.paidUntil||''); let cur=pu?addMonthsToYYYYMM(pu,1):(normalizeYYYYMM(p.createdAt||sel)||sel); const out=[]; let guard=0; while(cur&&cur<=sel&&guard<36){ if(!skipped.includes(cur)&&!paidMonths.includes(cur)) out.push(cur); cur=addMonthsToYYYYMM(cur,1); guard++; } if(p.isOwed===true&&Array.isArray(p.owedMonths)){ p.owedMonths.map(normalizeYYYYMM).filter(Boolean).forEach(m=>{ if(m<=sel&&!skipped.includes(m)&&!paidMonths.includes(m)&&!out.includes(m)) out.push(m); }); out.sort(); } return out; }
...
check('Dynamic: paidUntil April with paidMonths June only still owes May', JSON.stringify(chargeable({paidUntil:'Tháng tư 2026', paidMonths:['2026-06']}, '2026-06')) === JSON.stringify(['2026-05']));
check('Dynamic: paidMonths 2026-06 correctly suppresses June debt', chargeable({paidUntil:'2026-05', paidMonths:['2026-06']}, '2026-06').length === 0);
