...
check('duplicate registration has one stale-owner cleanup path', helper.includes("window.removeListener(clubKey, 'v5u6b-stale-bootstrap-owner')"));
check('duplicate registration remount is bounded to one explicit retry', helper.includes('registered = !existing ? false : register();') && !helper.includes('while (registered'));
...
check('no polling/retry interval in club bootstrap helper', !helper.includes('setInterval(') && !helper.includes('setTimeout('));
