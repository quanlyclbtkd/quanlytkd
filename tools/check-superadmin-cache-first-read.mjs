...
c('stats read is guarded by incomplete root cache', /if \(!_rootMonthCache\.complete\)\s*\{[\s\S]{0,500}getDoc\(doc\(db, 'clubs', cid, 'stats'/.test(s));
...
function root(data,m){const d=m.replace('-','_'), sa=data.superAdminStats||{}; const student=finite(sa.activeCount,sa.profileCount,data.cachedActiveCount,data.cachedStudentCount,data.cachedProfileCount,data.profileCount,data.totalStudents); const k=finite(keyed(data.cachedMonthlyRevenue,m,d),keyed(data.revenueByMonth,m,d)); const marker=String(sa.month||'').replace('_','-').slice(0,7); const rev=k!==null?k:(marker===m?finite(sa.revenueTotal,data.cachedCurrentMonthRevenue,data.currentMonthRevenue):null); return {student,revenue:rev,complete:student!==null&&rev!==null};}
const m='2026-08';
