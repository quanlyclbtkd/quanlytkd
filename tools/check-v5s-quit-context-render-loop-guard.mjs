...
check('V5S/V5T cache-bust lineage active', (index.includes(`app.js?v=${currentBuild}`) || index.includes(`app.js?v=${build}`)) && (index.includes(`js/main.js?v=${currentBuild}`) || index.includes(`js/main.js?v=${build}`)) && main.includes(`profiles.listeners.js?v=${build}`));
check('V5S behavior retained under V5T patch marker', (app.includes(currentPatch) || app.includes(patch)) && (main.includes(currentPatch) || main.includes(patch)) && boundary.includes(patch));
