...
const runtimeBuild = 'coach-attendance-branch-hydration-20260630-v4d6';
...
check('index loads profileCanonicalStore after tuitionDebtCanonical and before app.js',
  ...
  index.indexOf(`js/core/profileCanonicalStore.js?v=${build}`) < index.indexOf(`app.js?v=${runtimeBuild}`));
check('index cache-busts app.js and main.js to current runtime build', index.includes(`app.js?v=${runtimeBuild}`) && index.includes(`./js/main.js?v=${runtimeBuild}`));
check('main.js retains V4D1 lineage marker', main.includes(`APP_BUILD_VERSION = '${runtimeVersion}'`) || main.includes(`APP_PATCH_VERSION = '${runtimeVersion}'`) || main.includes("APP_PATCH_VERSION = '4K-6V4D2-mobile-small-ui-recovery-20260628'") || main.includes("APP_PATCH_VERSION = '4K-6V4D4-quit-authoritative-full-sync-20260629'") || main.includes(`APP_BUILD_VERSION = '${version}'`) || main.includes(`APP_PATCH_VERSION = '${version}'`));
