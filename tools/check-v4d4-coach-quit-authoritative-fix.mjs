...
check('APP_PATCH_VERSION marks V4D4 runtime',
  (files.main.includes("APP_PATCH_VERSION = '4K-6V4D4-coach-quit-authoritative-fix-20260630'") || files.main.includes("APP_PATCH_VERSION = '4K-6V4D5-coach-quit-attendance-full-recovery-20260630'")) &&
  (files.main.includes("window.APP_PATCH_VERSION = '4K-6V4D4-coach-quit-authoritative-fix-20260630'") || files.main.includes("window.APP_PATCH_VERSION = '4K-6V4D5-coach-quit-attendance-full-recovery-20260630'")));
...
check('Coach active listener reads branch aliases without using where-in on branch',
  files.profiles.includes('_coachBranchAliases(context)') &&
  files.profiles.includes('coach-branch-alias-compat') &&
  files.profiles.includes("fbWhere('branch', '==', alias)") &&
